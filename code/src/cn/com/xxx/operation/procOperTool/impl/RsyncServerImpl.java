...
/**
 * RsyncServer进程操作类，继承基类ProcOperTool
 * @author zh
 * 
 */
public class RsyncServerImpl extends ProcOperTool
{
    @Override
    public String query()
    {
        final String[] command = { "/bin/sh", "-c",
                "ps -ef | grep " + StringInfo.RsyncServer_procName + " | grep -v grep" };
    ...
    public String start()
    {
        final String[] command = { "/bin/sh", "-c", StringInfo.RsyncServer_start };
    ...
    public String close()
    {
        final String[] command1 = {
                "/bin/sh",
                "-c",
                "ps -ef | grep " + StringInfo.RsyncServer_procName
