...
/**
 * ProxyFileServer进程操作类，继承基类ProcOperTool
 * @author zh
 * 
 */
public class ProxyFileServerImpl extends ProcOperTool
{
    @Override
    public String query()
    {
        final String[] command = {
                "/bin/sh",
                "-c",
                "ps -ef | grep " + StringInfo.ProxyFileServer_procName
                        + " | grep -v grep" };
    ...
    public String start()
    {
        final String[] command = { "/bin/sh", "-c", StringInfo.ProxyFileServer_start };
    ...
    public String close()
    {
        final String[] command = { "/bin/sh", "-c", StringInfo.ProxyFileServer_close };
        return setOperResult(execRes(command));
    }
